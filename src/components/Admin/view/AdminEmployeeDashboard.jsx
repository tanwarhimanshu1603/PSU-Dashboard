...
export default function EmployeeDashboard() {
    ...
    useEffect(() => {
        ...
        const fetchEmployeeDetails = async () => {
        try {
            const response = await fetch(`${GLOBAL_CONFIG.BASE_URL}api/v1/admin/getById/${empId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': jwtToken 
    ...
    const getAllSkills = async()=>{
        try {
        // First API call to admin login
        const skillResponse = await fetch(`${GLOBAL_CONFIG.BASE_URL}api/v1/admin/getSkills`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization':jwtToken
    ...
    const getAllDomains = async()=>{
        try {
        const domainResponse = await fetch(`${GLOBAL_CONFIG.BASE_URL}api/v1/admin/getDomain`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization':jwtToken
    ...
    const updateDetails = async () => {
        setLoading(true)
        try {
          const response = await fetch(`${GLOBAL_CONFIG.BASE_URL}api/v1/admin/update`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
    ...
    return (
        <Box sx={{ p: 3, bgcolor: '#f7f8fc', minHeight: '100vh' }}>
            ...
            >
                ...
                <Box>
                    <Avatar aria-controls={open ? 'basic-menu' : undefined} aria-haspopup="true" aria-expanded={open ? 'true' : undefined} onClick={handleImgDropDown} alt={employee.empName} src={employee.empImage} sx={{cursor:"pointer"}} />
                </Box>
            </Box>
                                            ...
                                            <Typography sx={{fontWeight: 'bold',color: '#333333'}}>
                                                {
                                                    updateMode ? 
                                                    <TextField onChange={handleChange} id="standard-basic" name='currentAccount' value={employee.currentAccount} variant="standard" /> 
                                                    : employee.currentAccount && employee.currentAccount==="undefined"?"":employee.currentAccount
                                                }
                                            </Typography>
                                        </Grid>
                                        <Grid size={{xs:4,sm:4,md:4}}>
